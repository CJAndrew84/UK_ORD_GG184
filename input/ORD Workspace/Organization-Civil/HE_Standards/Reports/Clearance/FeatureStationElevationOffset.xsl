        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:for-each select="InRoads/GeometryProject">
                    <xsl:text/>
                    <xsl:text>Chainage Elevation Offset Report&#xd;&#xd;</xsl:text>
                    <xsl:for-each select="$uniqueClearanceAlignments">
                        <xsl:value-of select="."/><xsl:text>&#xd;</xsl:text>
                        <xsl:text>     Chainage           Elevation          Offset        Surface&#xd;</xsl:text>
                        <xsl:text>     ---------         -----------        --------      ---------&#xd;&#xd;</xsl:text>
                        <xsl:for-each select="//ClearancePoint[@clearanceAlignmentName = current()]">
                            ...
                            <xsl:value-of select="cif:columnFormat(cif:distanceFormat(number(@firstOffset)), 16)"/>
                            <xsl:text> &#xa0; &#xa0; &#xa0;</xsl:text>
                            <xsl:value-of select="@objectParentName"/>
                            <xsl:text>&#xd;</xsl:text>
                        </xsl:for-each>
                        <xsl:text>&#xd;</xsl:text>
                    </xsl:for-each>
