...
<xsl:stylesheet version="1.1" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:cif="cif">
                    ...
                    <xsl:for-each select="GeometryProject">
                        ...
                        <xsl:for-each select="HorizontalAlignment">
                            ...
                            <xsl:if test="@eastingClosingError">
                                ...
                                <xsl:text>        Y Error:  </xsl:text>
                                <xsl:value-of select="cif:columnDoubleFormat(number(@northingClosingError), 12)"/>
                                <xsl:text> </xsl:text>
                                <xsl:value-of select="$linearUnit"/>
                                ...
                                <xsl:value-of select="$linearUnit"/>
                                <xsl:text>&#xd;        Precision:  </xsl:text>
                                <xsl:value-of select="cif:columnDoubleFormat(number(@closingPrecision), 12)"/>
                            </xsl:if>
                            <xsl:text>&#xd;</xsl:text>
                        </xsl:for-each>
                    </xsl:for-each>
    ...
    <xsl:template match="HorizontalCircle">
        ...
        <xsl:text>)</xsl:text>
        <xsl:value-of select="cif:columnDoubleFormat(number(Center/@easting), 44)"/>
